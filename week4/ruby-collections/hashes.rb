...
# or
users[ "Erik" ][ :favorite_numbers ].sort.first  # more work, multiple iterations to sort!
...
faves = users.values.first[ :favorite_numbers ]  # initialise to first array, instead
users.values.each do |info|
  puts "comparing #{faves.to_s} with #{ info[:favorite_numbers].to_s }"
  faves = faves & info[ :favorite_numbers ]
end

# annoyingly magical one-liner
users.values.map { |data| data[:favorite_numbers]  }.reduce &:&
